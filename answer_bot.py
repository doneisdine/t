...
oot_channel_id_list = [
    "608671267729702959",#add the channels you want to fetch answers from
]
...
class Bot(discord.Client):
    ...
    async def on_ready(self):
        ...
        await self.change_presence(activity=discord.Game(name='with Just for fun | -c'))
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('SELF BOT TOKEN',
                                   bot=False))
    loop.run_forever()
